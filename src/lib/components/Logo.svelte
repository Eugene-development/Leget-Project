<script>
	/**
	 * Logo component - Full SVG logo with text
	 * @component
	 * 
	 * Props:
	 * - invert: boolean - Invert colors (white on dark background)
	 * - filled: boolean - Show filled state for the logomark
	 * - fillOnHover: boolean - Enable fill animation on hover
	...
	// Generate unique ID for clip-path to avoid conflicts when multiple logos are on the page
...
<svg
	viewBox="0 0 130 32"
	aria-hidden="true"
	class="{fillOnHover ? 'group/logo' : ''} {className}"
	{...restProps}
>
	<!-- Embedded Logomark -->
	<svg viewBox="0 0 32 32" preserveAspectRatio="xMinYMid meet">
		<rect
			clip-path="url(#{id}-clip)"
			class="h-8 transition-all duration-300 {invert ? 'fill-white' : 'fill-neutral-950'} {filled ? 'w-8' : 'w-0 group-hover/logo:w-8'}"
		/>
		<use
			href="#{id}-path"
			class={invert ? 'stroke-white' : 'stroke-neutral-950'}
			fill="none"
			stroke-width="1.5"
		/>
		<defs>
	...
	<!-- Text "studio." -->
	<path
		class={invert ? 'fill-white' : 'fill-neutral-950'}
