<script>
	...
	/**
	 * StylizedImage component - Displays images with decorative parallelogram clip paths
	 * Features hover animation that scales the image
	 *
	 * @prop {number} shape - Shape variant index (0, 1, or 2) - default: 0
	 ...
	 * Requirements: 12.8
	 */
	let { shape = 0, src, alt = '', class: className = '', ...restProps } = $props();
	...
	// Shape definitions with different parallelogram paths
	const shapes = [
		{
			width: 655,
			height: 680,
	...
	const selectedShape = $derived(shapes[shape] || shapes[0]);
</script>

<div class="relative flex aspect-719/680 w-full grayscale {className}" {...restProps}>
	{#if id}
				...
				>
					<foreignObject width={selectedShape.width} height={selectedShape.height}>
						<img
							...
							style="aspect-ratio: {selectedShape.width} / {selectedShape.height};"
							loading="lazy"
						/>
					</foreignObject>
				</g>
				<use href="#{id}-shape" stroke-width="2" class="stroke-neutral-950/10" />
			</g>
			<defs>
				<clipPath id="{id}-clip">
					<path id="{id}-shape" d={selectedShape.path} fill-rule="evenodd" clip-rule="evenodd" />
				</clipPath>
			</defs>
		</svg>
	{/if}
