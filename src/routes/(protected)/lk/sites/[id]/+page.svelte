<script>
	...
	async function handleSubmit(e) {
		...
		if (formName) {
			if (formName.length > 40) {
				fieldErrors.name = 'Название проекта не может превышать 40 символов';
				isSaving = false;
				return;
			}
			const hasLongWord = formName.split(/\s+/).some((word) => word.length > 20);
			if (hasLongWord) {
				fieldErrors.name = 'Одно слово в названии не может превышать 20 символов';
				isSaving = false;
				return;
			}
		}
	...
	function generateFaviconFromLogo(file) {
		return new Promise((resolve, reject) => {
			const reader = new FileReader();
			reader.onload = (e) => {
				const img = new Image();
				img.onload = () => {
					try {
						...
						const ctx = canvas.getContext('2d');
						if (!ctx) {
							reject(new Error('Не удалось создать контекст рисования'));
							return;
						}
						// Clear canvas (transparent background)
						ctx.clearRect(0, 0, 32, 32);

						// Draw centered with aspect ratio preserved
						const maxDim = Math.max(img.width, img.height);
						const w = (img.width / maxDim) * 32;
						const h = (img.height / maxDim) * 32;
						const x = (32 - w) / 2;
						const y = (32 - h) / 2;
	...
	async function handleLogoUpload(e) {
		...
		try {
			// 0. Compress Logo Image (skip SVGs since they are vector)
			let logoFile = file;
			if (file.type !== 'image/svg+xml') {
				try {
					logoFile = await compressLogoImage(file);
				} catch (compErr) {
					console.warn('Failed to compress logo, using original:', compErr);
				}
			}
			...
			if (!logoResponse.ok) throw new Error('Ошибка при загрузке логотипа в хранилище');
			formLogoUrl = logoUploadData.objectUrl;
			...
			if (faviconBlob) {
				...
				if (faviconResponse.ok) {
					formFaviconUrl = faviconUploadData.objectUrl;
				} else {
				...
				<div class="flex max-w-3xl flex-col gap-12">
					...
					<div class="flex items-center justify-between gap-4">
						<div class="min-w-0 flex-1">
							<h2 class="font-display text-3xl font-semibold break-words text-neutral-950">
								{license.name || 'Новый сайт'}
							</h2>
							{#if license.templateId}
								<p class="mt-1 text-xs text-neutral-400">
									Шаблон: <span class="font-medium text-neutral-600"
										>{TEMPLATE_NAMES[license.templateId] ?? `#${license.templateId}`}</span
									>
								</p>
							{/if}
							{#if license.domain && !license.domain.includes('pending-')}
								<p class="mt-1 text-xs text-neutral-400">
									Домен: <span class="font-medium text-neutral-600">{license.domain}</span>
								</p>
							{/if}
						</div>
						...
						<section class="rounded-3xl border border-neutral-100 bg-neutral-50/50 p-6 sm:p-8 shadow-sm ring-1 ring-neutral-950/5 space-y-6">
							...
							<div class="group relative">
								...
								<div class="mt-2 flex items-center gap-6">
									...
									>
										{#if formLogoUrl}
											<img src={formLogoUrl} alt="Logo" class="h-12 w-12 object-contain" />
										{:else}
											<svg
						...
						<section class="rounded-3xl border border-neutral-100 bg-neutral-50/50 p-6 sm:p-8 shadow-sm ring-1 ring-neutral-950/5 space-y-6">
							<div>
								...
								<p class="mt-1 text-sm text-neutral-500">
									Включайте и отключайте разделы каталога. Отключенные категории не будут
									отображаться на публичном сайте, но все связанные данные сохранятся.
								</p>
							</div>
							...
							{:else}
								<div class="grid grid-cols-1 gap-6 md:grid-cols-2">
									{#each rubrics as rubric (rubric.id)}
										<div
											class="rounded-3xl border border-neutral-100 bg-white p-6 shadow-sm ring-1 ring-neutral-950/5"
										>
											<h4
												class="font-display mb-4 flex items-center justify-between text-base font-semibold text-neutral-900"
											>
												...
												<div class="divide-y divide-neutral-100/80">
													{#each rubric.categories as category (category.id)}
														...
														>
															...
															<button
																type="button"
																class="relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:ring-2 focus:ring-neutral-950 focus:ring-offset-2 focus:outline-none {category.is_enabled
																	? 'bg-neutral-900'
																	: 'bg-neutral-200'}"
																role="switch"
																...
																onclick={() =>
																	handleToggleCategory(category.id, category.is_enabled)}
															>
																<span class="sr-only">Toggle category enabled state</span>
																<span
																	class="pointer-events-none relative inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out {category.is_enabled
																		? 'translate-x-5'
																		: 'translate-x-0'}"
																>
					...
					{#if license.status !== 'cancelled'}
						<div class="rounded-3xl border border-red-100 bg-red-50/50 p-6">
							...
							<p class="mt-2 text-sm text-red-700">
								Отказ от лицензии деактивирует сайт и остановит ежедневное списание средств. Это
								действие необратимо.
							</p>
							<div class="mt-4">
								<button
									...
									class="inline-flex items-center gap-2 rounded-2xl border border-red-300 bg-white px-4 py-2.5 text-sm font-semibold text-red-700 transition hover:border-red-400 hover:bg-red-50 focus:ring-2 focus:ring-red-500 focus:ring-offset-2 focus:outline-none"
								>
									<svg
										xmlns="http://www.w3.org/2000/svg"
										class="size-4"
	...
	>
		<div class="w-full max-w-md rounded-3xl bg-white p-8 shadow-2xl">
			...
			<p class="mt-3 text-sm text-neutral-600">
				Сайт <span class="font-semibold text-neutral-950">{license?.name || 'Новый сайт'}</span> будет
				деактивирован, а ежедневное списание средств — остановлено. Восстановить лицензию самостоятельно
				будет невозможно.
			</p>
			...
			<div class="mt-6 flex gap-3">
				<button
					...
					class="flex-1 rounded-2xl bg-red-600 px-4 py-3 text-sm font-semibold text-white transition hover:bg-red-700 focus:ring-2 focus:ring-red-500 focus:ring-offset-2 focus:outline-none disabled:opacity-60"
				>
